    ...
    if all_rows > 0 and recstart == 1:
        ...
        if isInitialized:
            ...
            pm.move(float((float(dataarray[5]) - float(PRE_name_downLip_ctrl_X)) * strengthX) * (-1),
                    float((float(dataarray[4]) - float(PRE_name_downLip_ctrl_Y)) * strengthY) * (-1),       # y는 왜 굳이 -1 곱했었지...?
                    float((-float(dataarray[3]) - float(PRE_name_downLip_ctrl_Z)) * strengthZ),     # 여기도 -1 곱해야 할까?
            ...
            pm.move((float(dataarray[20]) - float(PRE_name_l_Nose_ctrl_X)) * strengthX,
                    ...
                    (-float(dataarray[18]) - float(PRE_name_l_Nose_ctrl_Z)) * strengthZ,
                    'name_l_Nose_ctrl', relative=True, objectSpace=True, worldSpaceDistance=True)
            ...
            pm.move((float(dataarray[29]) - float(PRE_name_r_up_cheek_ctrl_X)) * strengthX,
                    (float(dataarray[28]) - float(PRE_name_r_up_cheek_ctrl_Y)) * strengthY,
                    (-float(dataarray[27]) - float(PRE_name_r_up_cheek_ctrl_Z)) * strengthZ,
                    'name_r_up_cheek_ctrl', relative=True, objectSpace=True, worldSpaceDistance=True)

            PRE_name_r_up_cheek_ctrl_Z = -float(dataarray[27])
            ...
            pm.move((float(dataarray[32]) - float(PRE_name_r_cheek_ctrl_X)) * strengthX,
                    (float(dataarray[31]) - float(PRE_name_r_cheek_ctrl_Y)) * strengthY,
                    (-float(dataarray[30]) - float(PRE_name_r_cheek_ctrl_Z)) * strengthZ,
                    'name_r_cheek_ctrl', relative=True, objectSpace=True, worldSpaceDistance=True)

            PRE_name_r_cheek_ctrl_Z = -float(dataarray[30])
            ...
            pm.move((float(dataarray[35]) - float(PRE_name_r_Nose_ctrl_X)) * strengthX,
                    (float(dataarray[34]) - float(PRE_name_r_Nose_ctrl_Y)) * strengthY,
                    (-float(dataarray[33]) - float(PRE_name_r_Nose_ctrl_Z)) * strengthZ,
                    'name_r_Nose_ctrl', relative=True, objectSpace=True, worldSpaceDistance=True)

            PRE_name_r_Nose_ctrl_Z = -float(dataarray[33])
            ...
            pm.move((float(dataarray[38]) - float(PRE_name_r_Lip_ctrl_X)) * strengthX,
                    (float(dataarray[37]) - float(PRE_name_r_Lip_ctrl_Y)) * strengthY,
                    (-float(dataarray[36]) - float(PRE_name_r_Lip_ctrl_Z)) * strengthZ,
                    'name_r_Lip_ctrl', relative=True, objectSpace=True, worldSpaceDistance=True)

            PRE_name_r_Lip_ctrl_Z = -float(dataarray[36])
            ...
            pm.move((float(dataarray[41]) - float(PRE_name_r_jaw_cheek_ctrl_X)) * strengthX,
                    (float(dataarray[40]) - float(PRE_name_r_jaw_cheek_ctrl_Y)) * strengthY,
                    (-float(dataarray[39]) - float(PRE_name_r_jaw_cheek_ctrl_Z)) * strengthZ,
                    'name_r_jaw_cheek_ctrl', relative=True, objectSpace=True, worldSpaceDistance=True)

            PRE_name_r_jaw_cheek_ctrl_Z = -float(dataarray[39])
            ...
            pm.move(float((float(dataarray[44]) - float(PRE_name_upLip_ctrl_X)) * strengthX) * (-1),
                    (float(dataarray[43]) - float(PRE_name_upLip_ctrl_Y)) * strengthY,
                    (-float(dataarray[42]) - float(PRE_name_upLip_ctrl_Z)) * strengthZ,
            ...
            PRE_name_upLip_ctrl_Z = -float(dataarray[42])
            ...
            pm.move(float((float(dataarray[47]) - float(PRE_name_l_upLip_ctrl_X)) * strengthX) * (-1),
                    (float(dataarray[46]) - float(PRE_name_l_upLip_ctrl_Y)) * strengthY,
                    (-float(dataarray[45]) - float(PRE_name_l_upLip_ctrl_Z)) * strengthZ,
                    'name_l_upLip_ctrl', relative=True, objectSpace=True, worldSpaceDistance=True)

            PRE_name_l_upLip_ctrl_Z = -float(dataarray[45])
            ...
            pm.move((float(dataarray[50]) - float(PRE_name_r_upLip_ctrl_X)) * strengthX,
                    (float(dataarray[49]) - float(PRE_name_r_upLip_ctrl_Y)) * strengthY,
                    (-float(dataarray[48]) - float(PRE_name_r_upLip_ctrl_Z)) * strengthZ,
                    'name_r_upLip_ctrl', relative=True, objectSpace=True, worldSpaceDistance=True)

            PRE_name_r_upLip_ctrl_Z = -float(dataarray[48])
